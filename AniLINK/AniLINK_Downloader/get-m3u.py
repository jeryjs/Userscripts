...
def get_output_file(link_info, folder):
...
def check_existing_files(links, folder):
    ...
    for idx, link_info in enumerate(links):
        output_file = get_output_file(link_info, folder)
    ...
    if existing_files:
        ...
        new_links = []
        for idx, link_info in enumerate(links):
            if (idx + 1) in overwrite_indices or not os.path.exists(get_output_file(link_info, folder)):
                new_links.append(link_info)
        return new_links
    return links

def download_stream(link_info, folder, progress_group, settings):
    ...
    name_without_ext = sanitize_filename(name_without_ext)
    output_file = os.path.join(folder, f"{name_without_ext}{ext}")
            ...
            while True:
                ...
                if 'Duration' in line:
                    ...
                    progress_group.update(
                        task,
                        completed=min(progress, 100),
                        description=(f"[cyan]{output_file} - {size_mb:.1f}MB @ {speed:.2f}x ({int(current_time)}/{int(duration)}s) [~{int(remaining)}s][/cyan]")
                    )
        ...
        with Progress(
            ...
            for link_info in links:
                ...
                thread = threading.Thread(
                    target=lambda sem, ln: (download_stream(ln, folder, progress, settings), sem.release()),
                    args=(semaphore, link_info)
                )
